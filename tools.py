...
class Assign_Graph():
    ...
    def get_dists_between_vars(self, vars, adj_list):
        ...
        It should be noted that if assign a = b and assign a = c, then dists[b][c] will not be considered in this case(it would be inf if no other edges between them).
        Returns:
    ...
    def run(self, expressions):
        ...
        return dists
    def get_vars_in_range(self, dists, start_var, range):
        """
        Get all vars that are reachable from start_var within the given range in BFS order.
        
        Args:
            dists (dict): the distance between each pair of variables
            start_var (str): the starting variable name
            range (int): the range to search for reachable variables
        Returns:
            list: a list of variables sorted by the level
        """
        if start_var not in dists:
            print("Error: start_var not in dists. Please check your input.")
            return []
        varlist = sorted([(var, dist) for var, dist in dists[start_var].items() if dist <= range], key=lambda x: x[1])
        return [var for var, _ in varlist]
    def get_code_for_vars(self, expressions, codes, vars):
        """
        Get the code for the given variables in the expressions.
        Note that due to the laziness of coder, it only consider the case where the whole expression is in the same line with "=" mark. Any code in the second line or below will be discarded.
        Note that due to the laziness of coder, the declarations are not discarded.
        
        Args:
            expressions (list): a list of expressions
            codes (list): a list of codes indexed with lineno-1
            vars (list): a list of variable names
        Returns:
            list: a list of codes for the given variables
        """
        code = []
        for i, e in enumerate(expressions[1:]):
            if e.father == None:
                left = expressions[e.left]
                line = e.line
                if left.name in vars:
                    code.append(codes[line-1].strip())
        return code
...
class MyParser(argparse.ArgumentParser):
    ...
    def __init__(self):
        ...
        debug_group.add_argument('--get-error-ios', action='store_true', help='Get error IOs')
        debug_group.add_argument('--get-all-ios', action='store_true', help='Get all IOs')
        debug_group.add_argument('--print-ops', action='store_true', help='Print all operations in the code')
        debug_group.add_argument('--get-time', action='store_true', help='Get time of analysis')
        debug_group.add_argument('--get-code-in-range', type=str, help='The base var to print code in a certain range. If --dist-factor-zero is given then use it as range. Else the default is 5')
        
        code_group = self.add_argument_group('Code Generation Options')
        modified_type_group = code_group.add_mutually_exclusive_group()
        modified_type_group.add_argument('--get-modified-code', action='store_true', help='Get modified code')
        modified_type_group.add_argument('--add-variables', action='store_true', help='Add middle vars into code')
        code_group.add_argument('--modify-type', type=int, default=2, help='The type of code modification. No effect for --add-variable flag 1: consider all vars in one assignment 2: Only add one branch when needed')
        
        analyse_group = self.add_argument_group('Analysis Options')
        load_store_group = analyse_group.add_mutually_exclusive_group()
        analyse_group.add_argument('--analyse-result', action='store_true', help='Run the simulation and analyse the result')
        load_store_group.add_argument('--store', action='store_true', help='Store the analyzer object to a pickle file')
        load_store_group.add_argument('--analyse-only', action='store_true', help='Analyse the result only, do not run the simulation. Conflicted with get_all_ios and get_error_ios')
        analyse_group.add_argument('--faulty-vars', nargs='*', default=['n_carry_out'], help='The faulty variable to analyse')
        analyse_group.add_argument('--analyser-type', type=int, default=2, help='Set the type of analyzer')
        analyse_group.add_argument('--analyse-output-file', type=str, default=None, help='The output file name for the analysis results. If None, the name will be generated by input file name and io name')
        analyse_group.add_argument('--vars-limit', type=int, default=10, help='The number of variables in the analysed list to be considered')
        analyse_group.add_argument('--dist-factor-zero', type=int, default=5, help='Parameter for the distance factor for analysis')
...
class Output_helper():
    ...
    """
    def __init__(self, output_file_name, output_dir="./output/"):
        """
...
class Fault_Locate_Analyzer_Factory():
    ...
    def create(self, dists, vars_limit=10, dist_factor=lambda x, d: x * 0.9 ** d if d != float('inf') else 0, type=3):
        ...
        if type == 1:
            ...
            analyzer = Fault_Locate_Analyzer_4(dists, vars_limit, dist_factor)
        else:
...
class Fault_Locate_Analyzer_2(Fault_Locate_Analyzer):
    ...
    def analyse(self, faulty_var, located_var_lists):
        ...
        ret = 0.0
        try:
            ret = max(self.dist_factor(w/max(w for (s, w) in self.sorted_var_lists[:self.vars_limit]), abs(self.dists[self.faulty_var][s])) if self.faulty_var in self.dists and s in self.dists[self.faulty_var] and self.dists[self.faulty_var][s] != float('inf') else 0 for (s, w) in self.sorted_var_lists[:self.vars_limit])
        except Exception as e:
            with open("./error.log", "w") as f:
                f.write(f"Error: faulty_var: {faulty_var}. Skipping\n")
                f.write(f"{e}")
        return ret
...
class Fault_Locate_Analyzer_4(Fault_Locate_Analyzer):
    """
    Return the rank of the first located variable with non-zero dist-factor
    """
    def analyse(self, faulty_var, located_var_lists):
        self.faulty_var = faulty_var
        if not self.error_handler():
        ...
        self.sorted_var_lists = sorted(located_var_lists, key=lambda x: x[1], reverse=True)
        res = 0
        for i, (var, weight) in enumerate(self.sorted_var_lists):
            if self.dist_factor(weight, abs(self.dists[self.faulty_var][var])) > 0:
                res = i + 1
                break
        return res